  ...
  async run (message, args) {
    ...
    try {
      ...
      message.channel.send(new MessageEmbed()
        ...
        .setTitle(`:desktop: • ${message.language.get('MCUSER')[1]}`)
        .addField(`:label: • ${message.language.get('MCUSER')[2]}`, args[0])
        .addField(`:credit_card: • ${message.language.get('MCUSER')[3]}`, UUID.join('-'))
        .addField(`:credit_card: • ${message.language.get('MCUSER')[4]}`, bodyUuid.id)
        .addField(`:film_frames: • ${message.language.get('MCUSER')[5]}`, pseudoHistory.join(' \n'))
        .addField(`:busts_in_silhouette: • ${message.language.get('MCUSER')[6]}`, message.language.get('MCUSER_SKIN', `https://minotar.net/skin/${bodyUuid.id}`))
